...
public class NavGraph implements IndexedGraph<NavCell> {
    ...
    public NavGraph() {
        ...
        for (int i = 0; i < SIZE * SIZE; i++) {
            ...
            grid[indexX][indexZ] = new NavCell(indexX - (SIZE/2f) + 0.5f, 10.5f, indexZ - (SIZE/2f) + 0.5f, i, indexX, indexZ, 0, 1f, true);
    ...
    public NavCell getCell(int indexX, int indexZ) {
        return grid[indexX][indexZ];
    }

    public NavCell getCell(Vector3 pos) {
        int indexX = (int) MathUtils.clamp(pos.x + (NavGraph.SIZE/2f), 0, NavGraph.SIZE-1);
        int indexZ = (int) MathUtils.clamp(pos.z + (NavGraph.SIZE/2f), 0, NavGraph.SIZE-1);

        return getCell(indexX, indexZ);
    }
}
